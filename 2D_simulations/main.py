...
dt = 0.01        # Time step
T = 300 # 1000       # Total simulation time
...
va = 0.55   # Self-propulsion velocity - 30 microns/h - v_rms for AOUPs
...
update_interval = steps // 100  # update 10000 times total
...
for t in range(steps):
    ...
    if t % 5 == 0:
        ax.clear()
        ax.scatter(x, y, s=280, color="#FF74FF", alpha=0.75, ec="None", zorder=0)
        
        for i in range(N):
            ax.arrow(x[i], y[i], 1.5*va_x[i], 1.5*va_y[i], width=0.05, length_includes_head=True, 
                    head_width=0.2, head_length=0.2, fc='#F76C1E', ec='#F76C1E', zorder=2)
            ax.arrow(x[i], y[i], 1.5*vx[i], 1.5*vy[i], width=0.05, length_includes_head=True,
                    head_width=0.2, head_length=0.2, fc='#1FE213', ec='#1FE213', zorder=2)

        if num_connections > 0:
            i_masked = connect[:, 0]
            j_masked = connect[:, 1]
            # Build list of segments (line endpoints) for visualization
            # shape: (num_pairs, 2, 2)
            segments = np.stack([positions[i_masked], positions[j_masked]], axis=1)
            # Create LineCollection
            lc = LineCollection(segments, colors="white", linestyles="dashed",
                                linewidths=1.5, zorder=1)
            ax.add_collection(lc)
            # ax.plot(x, y, 'k--', linewidth=1, zorder=0)

        ax.text(2.5, 3.9, f"$n=${N}", fontsize=12, color='white')
        ax.text(-4.4, 3.9, r"$t=%d\,\mathrm{h}$" % (t*dt) , fontsize=12, color='white')

        ax.text(-2.7, -4.5, f"# of connections$=${num_connections}", fontsize=11, color='white')

        ax.set_ylim(-5, 5)
        ax.set_xlim(-5, 5) # default: 7,5
        ax.tick_params(axis='both', length=0, labelbottom=False, labelleft=False)
        plt.savefig(f"frames/{t}.png", dpi=300, bbox_inches='tight')
